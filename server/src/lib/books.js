...
const isbnSlug = (isbn) => `isbn-${isbn}`;
const gutenbergSlug = (id) => `gutenberg-${id}`;
...
const mapOpenLibraryDoc = (doc) => {
  ...
  const isbn13 =
    isbnList.find((i) => typeof i === 'string' && /^\d{13}$/.test(i)) ||
...
const mapGutenbergDoc = (doc) => ({
  ...
  cover_url: null, // cover fetched lazily via helper to avoid network cost during search
  source: 'gutenberg',
  readable: true,
  read_kind: 'gutenberg',
...
const upsertOpenLibraryBooks = (books) => {
...
const upsertGutenbergBooks = (books) => {
  if (books.length === 0) return Promise.resolve();
  return db.query(
