...
const getTextUrl = async (id, preferred) => {
  if (preferred) return preferred;
  // Standard Gutenberg file locations, newest cache first
  const candidates = [
...
// Strip Gutenberg's boilerplate header/footer, keeping only the book content
const extractBookText = (raw) => {
  let text = raw;
...
const search = async (q) => {
  const data = await fetchJson(`${GUTENDEX_URL}?search=${encodeURIComponent(q)}&page=1`);
  return data.results || [];
};

const getById = async (id) => {
  const data = await fetchJson(`${GUTENDEX_URL}/${id}`);
  return data;
};
