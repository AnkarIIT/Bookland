...
import { parseSlug, rowToBook, upsertGutenbergBooks } from '../lib/books';
import { validate, bookKeySchema, ValidatedRequest } from '../middleware/validate';

const router = Router();
const OL_BOOKS_URL = 'https://openlibrary.org/api/books';

async function fetchOpenLibraryDetail(isbn: string) {
  ...
  return {
    ...
    authors: (record.authors || []).map((a: any) => a.name).filter(Boolean),
    ...
    subjects: (record.subjects || []).map((s: any) => s.name).filter(Boolean),
...
async function upsertOpenLibraryDetail(book: any) {
  await db.query(
...
const getLocalBook = async (whereClause: string, params: unknown[]) => {
  const { rows } = await db.query(`SELECT ${DETAIL_FIELDS} FROM books ${whereClause} LIMIT 1`, params);
  if (rows.length === 0) return null;
  return { ...rowToBook(rows[0]), ...rows[0] };
};

router.get('/:key', validate(bookKeySchema, 'params'), async (req: ValidatedRequest, res: Response, next: NextFunction) => {
  try {
    const parsed = parseSlug(req.validatedParams!.key as string);
    ...
    try {
      localBook =
        parsed.kind === 'isbn'
          ? await getLocalBook('WHERE isbn_13 = $1', [parsed.id as unknown])
          : await getLocalBook('WHERE gutenberg_id = $1', [parsed.id as unknown]);
    } catch (dbErr) {
      console.error('Local book lookup failed, falling back upstream:', (dbErr as Error).message);
    }
    ...
    if (parsed.kind === 'gutenberg') {
      try {
        ...
        console.error(`Gutenberg detail fetch failed for ${parsed.id}:`, (err as Error).message);
      }
    } else {
      try {
        ...
        console.error(`Open Library detail fetch failed for ${parsed.id}:`, (err as Error).message);
      }
    }
