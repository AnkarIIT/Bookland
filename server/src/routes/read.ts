...
import { validate, gutenbergReadSchema, archiveReadSchema, ValidatedRequest } from '../middleware/validate';

const router = Router();

const gutenbergContentFromDb = async (id: number) => {
...
router.get('/gutenberg/:id', validate(gutenbergReadSchema, 'params'), async (req: ValidatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      console.error('Local content lookup failed:', (dbErr as Error).message);
    }
    ...
    const authorNames: string[] = (doc.authors || []).map((a: any) => a.name);
    try {
      await db.query(
        ...
        [id, doc.title || 'Unknown Title', authorNames, content, textUrl] as unknown[]
      );
    } catch (dbErr) {
      console.error('Failed to cache gutenberg content:', (dbErr as Error).message);
    }
    ...
    if ((error as Error).message && (error as Error).message.includes('status')) {
...
router.get('/archive/:id', validate(archiveReadSchema, 'params'), async (req: ValidatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      console.error('Local archive metadata lookup failed:', (dbErr as Error).message);
    }
